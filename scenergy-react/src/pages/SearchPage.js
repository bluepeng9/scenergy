...
const SearchPage=()=> {
    ...
    useEffect(() => {
        const test = async () => {
            ...
            console.log((await videoPostApi.searchVideoPost({"word": ""})).data.data.list);
            const responseVideoPosts = await videoPostApi.searchVideoPost({"word": ""});
              ...
              <div>
                  {searchUsers.map((result, index) => (
                      <a key={index} href={`/profile/${result.userId}`}>
                          <span>{result.url} {result.nickName}</span>
                      </a>
                  ))}
              </div>
          </div>
